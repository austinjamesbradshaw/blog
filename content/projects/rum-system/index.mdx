...
description: "A custom engineered TypeScript + AWS CloudWatch RUM system to increase my team's frontend app observability."
dateLaunched: "2025-04-16"
outcomes:
  [
    "<strong>Built a custom TypeScript RUM provider</strong> on top of AWS CloudWatch Logs to satisfy security requirements.",
...
skills: ["TypeScript", "AWS Cloudwatch"]
...
After the launch of a particular app encountered some unexpected problems **that our users had to catch for us**, I rallied the team to look at our current level of observability.
...
- **PostHog** - I was familiar with PostHog from my hobby projects (thanks for the free-tier) and was a fan. We couldn't use their cloud service, but I was curious if self-hosting would be a solution. While it's features were arguably the best, we ultimately decided we couldn't afford the implementation and maintenance cost of self-hosting. We needed something managed.
- **Grafana LGTM Stack** - A few other teams in the enterprise were making use of Grafana, so we figured that could be a good option, too. We found it could definitely do what we needed, but we again ran into concerns about implementation and maintenance costs self-hosting it within our small team.
- **AWS CloudWatch** - Our team is deep in the AWS ecosystem. It's where our infra lives... our backend team was using CloudWatch for thier observability... it was a natural consideration for us. Our access to the ecosystem allowed us to easily put together a proof of concept using CloudWatch Logs and a dashboard.
...
**2) It had to comply with CloudWatch Log's rate limit.** We'd send our logs through `PutLogEvents` commands directly to the CloudWatch API, so we're limited to 5,000 transactions per second.
